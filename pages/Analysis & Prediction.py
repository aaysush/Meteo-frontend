...
if get_results:
    ...
    try:
        # Log the API request
        logging.info(f"Sending request to backend with data: {data}")

        # API call
        response = requests.post('http://127.0.0.1:8000/process-data', json=data)

        # Log the response status
        logging.info(f"Response status code: {response.status_code}")

        if response.status_code == 200:
            st.sidebar.success(f'Check the Analysis & Prediction tab for results.')
            result_data = response.json()  # Extract JSON data
            result_df = pd.DataFrame(result_data)  # Convert to DataFrame
...
# Display DataFrame or error message
